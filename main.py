...
def facesdetecter_init():
    ...
    image = cv2.imread(r"images/backgound.jpg")
...
def facesdetecter(image):
    ...
    thresh_h = cv2.inRange(H, minH, maxH)  # 0-180du 提取人体肤色区域 二值化
    ...
    if len(eyes) > 1:
        ...
        for (x, y, w, h) in rect_eyes:
            ...
            image1, contours, hierarchy= cv2.findContours(thresh_eyes, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)  # 寻找前景
            cv2.drawContours(image, contours, -1, (0, 255, 0), 3)
            for cont in contours:
                Area = cv2.contourArea(cont)  # 计算轮廓面积
                total_area_eyes += Area
        print("total_area_eyes=", total_area_eyes)
        frame = cv2.putText(image, "Eyes Area : {:.3f}".format(total_area_eyes), (120, 40), cv2.FONT_HERSHEY_COMPLEX,
                            0.5, (0, 255, 0), 1)  # 绘制
        ...
        for (x, y, w, h) in rect_mask:
            ...
            image1, contours, hierarchy = cv2.findContours(thresh_mask, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)  # 寻找前景
...
face_cascade.load(r"data/haarcascades/haarcascade_frontalface_default.xml")  # 一定要告诉编译器文件所在的具体位置
'''此文件是opencv的haar人脸特征分类器'''

# 主要还是通过眼睛位置进行区域判断
eyes_cascade = cv2.CascadeClassifier(r"haarcascade_eye_tree_eyeglasses.xml")
eyes_cascade.load(r"data/haarcascades/haarcascade_eye_tree_eyeglasses.xml")  # 一定要告诉编译器文件所在的具体位置
